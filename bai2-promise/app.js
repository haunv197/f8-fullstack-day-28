...
function sendRequestUserProfileCard(method, url, retry = 0, delay = 2000) {
  sendRequest(method, url)
    .then((users) => {
      renderUser(null, users)
    })
    .catch((error) => {
      renderUser(error, null)
      if (retry == 0) {
        retry++;
        setTimeout(() => {
          autoRetryUser.innerHTML = `Tự động retry: ${retry} lần`;
          autoRetryUser.classList.add("show");
          sendRequestUserProfileCard(method, url, retry)
        }, delay)
      }
...
searchUserBtn.addEventListener("click", function () {
  ...
  sendRequestUserProfileCard(METHOD_GET, `https://jsonplaceholder.typicode.com/users/${userId}`)
})
...
function handleShowComment(postID, userName) {
  ...
  sendRequest(METHOD_GET, `https://jsonplaceholder.typicode.com/posts/${postID}/comments`)
    .then((comments => {
...
function sendRequestLoadPosts(method, url, retry = 0, delay = 2000) {
  let postsData = [];
  ...
  sendRequest(method, url)
    .then((posts) => {
      ...
      postsData = posts;
      return sendRequest(method, "https://jsonplaceholder.typicode.com/users")
    })
    .then((users) => {
      postsError.classList.remove("show");
      renderPosts(postsData, users)
    })
    .catch((error) => {
      postsError.classList.add("show");
      postsErrorText.textContent = `${error.message} posts`;
      if (retry === 0) {
        retry++;

        setTimeout(() => {
          sendRequestLoadPosts(method, url, retry)
          console.log("retry posts", retry)
        }, delay)
      }
...
function loadPosts(numberPost) {
  ...
  sendRequestLoadPosts(METHOD_GET, `https://jsonplaceholder.typicode.com/posts?_start=${numberPost}&_limit=${LIMIT_POST_SHOW}`);
  return;
}
...
function sendRequestLoadTodos(method, url, userId, retry = 0, delay = 2000) {
  sendRequest(method, url)
    .then((todos) => {
      todosData = todos;
    ...
    .catch((error) => {
      ...
      if (retry === 0) {
        setTimeout(() => {
          sendRequestLoadTodos(method, url, userId, retry)
          console.log("retry todos", retry)
        }, delay)
      }
      return;
    }).finally(() => {
